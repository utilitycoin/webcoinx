...
//			 real.ins[i].script = Script.createInputScript(sig, pk);
     ...
     MockWallet.prototype.createPayment = function (color, amount, to_address) {
         ...
         var t = MockExchangeTransaction(this, 
                                            {
                                                tx: {inp: utxos.list.forEach.function(utxo) {return { outpoint: utxo.outpoint, utxo: utxo, signed: false};}},
                                                     out: [{to: to_address, value: amount, color: color}]},
                                                my: outpoints
                                            });
		 // if the total is larger, append sending change back
		 if (utxos.total > amount) {
			 t.tx.out.push({to: this.getAddress(), value: utxos.total - amount});
			 t.realtx = null;
		 }
		 return t;
     };
